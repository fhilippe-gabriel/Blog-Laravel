...
const Certificados = props => {
    ...
    const tableTitle = 'Lista de Certificados';
    const tableColumns = [
        {
            name: 'id',
            label: 'Id',
    ...
    useEffect(() => {
        props.onListCertificados()
    }, [])


    const handleEdit = (row) => {
        props.onInitUpdateCertificado(props.pagination.data[row]['id']);
    }

    const handleCreate = () => {
        props.onInitCreateCertificado()
    }

    const handleConfirm = (id) => {
        props.onRemoveCertificado(id);
    ...
    const handleDelete = (row) => {
        setDeleteDialogOpen(!deleteDialogOpen)
        setIdCertificado(props.pagination.data[row]['id'])
    }
    ...
    return (
        <div>
            ...
            />
            {deleteDialogOpen ? (<DeleteDialog id={idCertificado} handleConfirm={handleConfirm} />) : null}
        </div>
    );
}
...
const mapDispatchToProps = dispatch => ({
    onListCertificados: () => dispatch(actions.listCertificados()),
    paginate: (params) => dispatch(actions.paginate(params)),
    onFormSubmit: (values) => dispatch(actions.createCertificado(values)),
    onRemoveCertificado: (id) => dispatch(actions.removeCertificado(id)),
    onInitCreateCertificado: () => dispatch(actions.initCreateCertificado()),
    onInitUpdateCertificado: (id) => dispatch(actions.initUpdateCertificado(id)),
    closeNotif: () => dispatch(closeNotifAction),
})

const CertificadosMapped = connect(
    mapStateToProps,
    mapDispatchToProps
)(Certificados);


export default (withStyles(styles)(CertificadosMapped));